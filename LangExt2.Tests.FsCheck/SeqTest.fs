...
let some x = LangExt2.Option.Some(x)
let none () = LangExt2.Option<_>.None
// Result生成簡略化
let success x = LangExt2.Result.Success(x).CastFailure<_>()
let failure x = LangExt2.Result.Failure(x).Cast<_>()
...
let ``Create.Seq eq LangExtSeq.Create``() =
  test (fun (xs: int[]) -> LangExt2.Create.Seq(xs)) eq (intSeq)
...
let ``LangExtSeq.Sum(xs, SumStrategy.String) eq String.concat ""``() =
  test (strSeq >> (fun xs -> LangExtSeq.Sum(xs, LangExt2.SumStrategy.String))) (=) (String.concat "")
...
let ``LangExtSeq.SumBy(xs, SumStrategy.String) eq String.concat ""``() =
  test (fun xs -> LangExtSeq.SumBy(intSeq xs, (fun x -> string x), LangExt2.SumStrategy.String)) // LangExt側はFuncを要求するのでラムダ式が必須
...
let ``LangExtSeq.Sequence`` () =
  ...
  let optEq a b =
    let conv (x: LangExt2.Option<LangExt2.ISeq<_>>) =
...
let ``LangExtSeq.SequenceSuccess`` () =
  ...
  let resEq a b =
    let conv (x: LangExt2.Result<LangExt2.ISeq<_>, int>) =
...
let ``LangExtSeq.SequenceFailure`` () =
  let resSeq xs = LangExtSeq.Map(intSeq xs, fun x -> if even x then success(x) else failure(x))
  let resEq a b =
    let conv (x: LangExt2.Result<int, LangExt2.ISeq<_>>) =
...
let ``LangExtSeq.OnlySuccess eq (Seq.choose conv)``() =
  let conv (x: LangExt2.Result<_, _>) =
    if x.IsSuccess then Some(x.GetOrElse(fun () -> failwith ""))
    else None
  let resSeq xs = LangExtSeq.Map(intSeq xs, fun x -> if even x then success(x) else failure(x))
...
let ``LangExtSeq.MapResult (class) eq (Seq.map (function null -> failure(()) | x -> success(x)))``() =
  let strSeq xs = LangExtSeq.Map(strSeq xs, fun x -> if x.Length = 1 then null else x)
  test (strSeq >> LangExtSeq.MapResult) eq (strSeq >> (Seq.map (function null -> failure(LangExt2.Unit.``_``) | x -> success(x))))
...
let ``LangExtSeq.MapResult (nullable struct) eq (Seq.Map (function Null -> failure(()) | NotNull x -> success(x)))``() =
  ...
  test (nullableIntSeq >> LangExtSeq.MapResult)
    eq (nullableIntSeq >> (Seq.map (function Null -> failure(LangExt2.Unit.``_``) | NotNull x -> success(x))))